...
class Carousel extends React.Component{
    ...
    render(){
        ...
        return(
            <div>
                {this.state.isMobile && <p style={{textAlign: 'center', color: '#D6D6D6', marginBottom: 0, fontStyle: "italic", fontSize: '3.5vw'}}>*Click the logo to play the video.*</p>}
                {!this.state.isMobile && <p style={{textAlign: 'center', color: '#C6C6C6', marginBottom: 0, fontStyle: "italic", fontSize: '0.75vw'}}>*Hover over the logo to play the video.*</p>}
                <div id='test' className={!this.state.isMobile ? "carousel-container" : "carousel-container-mobile"}>
                    <motion.button  style={{marginRight: 25}} whileHover={{scale: 1.05, transition: {duration: 0.3}}} className={!this.state.isMobile ? "carousel-buttons" : "carousel-button-mobile"} onClick={()=>{this.decrease(); this.setState({fade: true})}}><BsArrowBarLeft className={!this.state.isMobile ? "carousel-button-icon" : "carousel-button-icon-mobile"}/></motion.button>
                    {items[this.state.currentItem]}
                    <motion.button style={{marginLeft: 25}} whileHover={{scale: 1.05, transition: {duration: 0.3}}} className={!this.state.isMobile ? "carousel-buttons" : "carousel-button-mobile"} onClick={()=>{this.increase(); this.setState({fade: true})}}><BsArrowBarRight className={!this.state.isMobile ? "carousel-button-icon" : "carousel-button-icon-mobile"}/></motion.button>
                </div>
            </div>
        );
    }
